...
Obecně však lze říct, že za nenalezené segmenty je zodpovědné velké množství šumu v datech. Tento šum jednak nedovoluje prohledávat akumulátorové pole příliš podrobně a zároveň šum způsobuje, že je akumulátorové pole větší, než by bylo potřeba. Toto tvrzení můžu podložit následujícím testem. Mračno stodůlky je rozděleno do tří souborů. Jeden z těchto souborů obsahuje polovinu bodů, ale naprostou většinu rovin (neobsahuje tedy tolik šumu). Pokud provedu segmentaci pouze na této části z celkového mračna, výsledky segmentace jsou výrazně lepší, viz tabulka \ref{table:test-st-2} a obrázek \ref{fig:test-st-2}. 
...
\end{center}
\end{figure}
V tomto případě byly nalezeny správně všechny rovinné segmenty kromě střechy vlevo nahoře, která obsahuje jen velmi málo bodů, navíc není příliš rovná.
\end{changeheight}
