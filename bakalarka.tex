...
Jako pokus jsem zvolil malé okolí bodu, hodnotu jsem nastavil na 10. Tím jsem docílil toho, že výpočet parametrů nebyl příliš přesný, identifikované segmenty měly tedy větší RMS a obvykle obsáhly více bodů ze svého okolí. Tím jsem chtěl dosáhnout toho, že když střechy nejsou příliš rovné, horší přesnost vypočtených parametrů by mohla částečně tento problém \uv{zakrýt}. Tato myšlenka se ve skutečnosti ukázala správná a opravdu bylo nalezeno méně segmentů, které více odpovídaly střechám. Pokud jsem při testování používal větší velikost okolí, algoritmus nacházel více segmentů, který byly sice relativně přesnější, ale méně odpovídaly \uv{reálným} rovinám.
